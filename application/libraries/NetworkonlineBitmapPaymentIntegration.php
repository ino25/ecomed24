...
class NetworkonlineBitmapPaymentIntegration {
	...
	public $billingDataBlock     = true; // Billing Data Block      ==> This is an optional block ,0 
	public $shippingDataBlock    = true; // Shipping Data Block     ==> This is an optional block ,0 
	...
	*/
	...
	public function __construct(){
		...
		if($this->billingDataBlock){
			...
			$emptybillingArray = isset($emptybillingArray) ? $emptybillingArray : array(); // New: zuulu (Debug undefined if billingDataBlock: false & empty($argumentsArray['Field_Existence_Indicator_Billing'])
		...
		if($this->shippingDataBlock){
			...
			$emptyshippingArray = isset($emptyshippingArray) ? $emptyshippingArray : array(); // New: zuulu (Debug undefined if shippingDataBlock: false & empty($argumentsArray['Field_Existence_Indicator_Shipping'])
			// Replace all the shippingArray Values to NULL and JOIN the Array with pipe delimiter
		...
		if($this->paymentDataBlock){
			...
			$emptypaymentArray = isset($emptypaymentArray) ? $emptypaymentArray : array(); // New: zuulu (Debug undefined if paymentDataBlock: false & empty($argumentsArray['Field_Existence_Indicator_Payment'])
			// Replace all the shippingArray Values to NULL and JOIN the Array with pipe delimiter
		...
		if($this->merchantDataBlock){
			...
			$emptymerchantArray = isset($emptymerchantArray) ? $emptymerchantArray : array(); // New: zuulu (Debug undefined if merchantDataBlock: false & empty($argumentsArray['Field_Existence_Indicator_Merchant'])
			// Replace all the merchantArray Values to NULL and JOIN the Array with pipe delimiter
			foreach( $this->merchantArray as $key => $val ){
		...
		if($this->otherDataBlock){
			...
			$emptyotherDataArray = isset($emptyotherDataArray) ? $emptyotherDataArray : array(); // New: zuulu (Debug undefined if otherDataBlock: false & empty($argumentsArray['Field_Existence_Indicator_OtherData'])
			// Replace all the merchantArray Values to NULL and JOIN the Array with pipe delimiter
		...
		// $this->beforeEncryptionString ="202008171000001||ORAB||1110000||11111111|DOC1604399934102|1|http://127.0.0.1:8080/ORAB/sampleClassSeven.php|http://127.0.0.1:8080/ORAB/sampleClassSeven.php|INTERNET|CC|01|XOF||1111111111110|Abdul|Kahar|aaa|aaa|aaa|aaa|aaa|AE|code.tester.123@some.mail.com|1234567890|1234567890|1234567890||111111111110|Abdul|Kahar|aaa|aaa|aaa|aaa|aaa|AE|1234567890|1234567890|1234567890";
		// $this->beforeEncryptionString ="202008171000001||ORAB||1110000||11111111|DOC1604399934102|1|http://127.0.0.1:8080/ORAB/sampleClassSeven.php|http://127.0.0.1:8080/ORAB/sampleClassSeven.php|INTERNET|CC|01|XOF||1111111111110|Abdul|Kahar|aaa|aaa|aaa|aaa|aaa|AE|code.tester.123@some.mail.com|1234567890|1234567890|1234567890||111111111110|Abdul|Kahar|aaa|aaa|aaa|aaa|aaa|AE|1234567890|1234567890|1234567890";
		
		
		//join('||',array_filter($this->DataBlocksArray));
		// echo "beforeEncryptionString: ".$this->beforeEncryptionString;
		...
		$this->AddLog('beforeEncryptionString : '.$this->merchantId.'||'.$this->collaboratorId.'||'.$this->beforeEncryptionString,'13');

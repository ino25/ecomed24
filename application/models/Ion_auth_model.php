...
class Ion_auth_model extends CI_Model
{
	...
	public function email_check_and_not_own($email = '', $user_id)
	{
		$this->trigger_events('email_check');
		...
		$array = array('email' => $email, 'id !=' => $user_id);

		return $this->db->where($array)->group_by("id")->order_by("id", "ASC")->limit(1)->count_all_results($this->tables['users']) > 0;
	}
	...
	public function register($username, $password, $email, $dfg, $additional_data = array(), $groups = array())
	{
		...
		if ($dfg != 5) {
			$data = array(
				...
				'active'     => ($email_activation === TRUE && $manual_activation === FALSE ? 0 : 1)
			);
	 ...
	 * @author Mathew
	 **/
	public function registerPatient($patient_id, $name, $last_name, $password, $email, $dfg, $phone, $address, $organisation, $patient_user_id, $additional_data = array(), $groups = array())
	{
		...
		$data = array(
			...
			'email' => $patient_id,
			'password'   => '$2y$08$sTs1ukNpuF/nxIIYwC2WYe9XV2IO4pZ.jESeLELDqXquomPG.m.S.',
			'ip_address' => $ip_address,
			'created_on' => time(),
			// 'active'     => ($manual_activation === FALSE ? 1 : 0)
			'active'     => '1'
		...
		//filter out any data passed that doesnt have a matching column in the users table
		//and merge the set user data and the additional data
	//	$user_data = array_merge($this->_filter_data($this->tables['appointmentcenterusers'], $additional_data), $data);
		...
		$this->home_model->insertAppointmentcenterusers($data);
	//	$this->home_model->insertPatientUsers($data);

		$id = $this->db->insert_id();

		$this->patient_model->updatePatient($patient_user_id, array('ion_user_id' => $id), $organisation);
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, phone, password, active, last_login, otp_validated, id_organisation')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
		...
		if ($query->num_rows() === 1) {
			...
			if ($password === TRUE) {
				...
				// BEGIN OF OTP RELATED CHANGES
				// Check to be done at this point forward
				// Check if last_login occured more than X ago
				$current_time = time();
				$last_login = $user->last_login;
				$time_since_last_login = $current_time - $last_login;
				// $min_time_before_otp_renewal = 12*60*60; // (12h)
				// $min_time_before_otp_renewal = 1*1*60; //(1minute)
				// $min_time_before_otp_renewal = 1*2*60; //(2 min)
				// $min_time_before_otp_renewal = 1*1*10; //(10sec)
				$min_time_before_otp_renewal = $this->config->item('min_time_before_otp_renewal', 'ion_auth');
				$otp_validated = $user->otp_validated;

				if (($time_since_last_login > $min_time_before_otp_renewal) && !$otp_validated) {

					$phone = $user->phone;
					$email = $user->email;
					$id = $user->id;
					$min_avant_expiration = $this->config->item('otp_expiration', 'ion_auth') / 60;

					$error_message = $this->error_start_delimiter . "<i class='fa fa-exclamation-triangle' style='font-size:30px;color:red;'></i><p style='width:80%;margin:auto;color:#000;text-align:center;font-size:95%;'>Vous devez vérifier votre identité.<br/>Un code de vérification valide pour " . $min_avant_expiration . " minutes vous a été envoyé par SMS au " . $this->maskAllButLast4($phone) . " et par email à l'adresse " . $this->maskEmail($email) . ".</p>" . $this->error_end_delimiter;
					$outcome = array(FALSE, array("error_message" => $error_message, "id" => $id, "phone" => $phone, "email" => $email, "password" => $password_entered));
					return $outcome;
				} else {
					// END OF OTP RELATED CHANGES

					$this->set_session($user);

					$this->update_last_login($user->id);
					$this->unvalidate_otp($user->id); // For next connection // OTP RELATED
					...
					return TRUE;
					// START OF OTP RELATED CHANGE (END OF ELSE CONDITION)
				}
				// END OF OTP RELATED CHANGE (END OF ELSE CONDITION)
			}
		}
	 ...
	 * @author Mathew
	 **/
	public function loginPatient($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select('username, email, id, phone, password, active, last_login, otp_validated, id_organisation')
			->where('id', $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
	...
	public function superlogin($identity, $password, $remember = FALSE)
	{
		$this->trigger_events('pre_login');
		...
		if ($query->num_rows() === 1) {
			...
			$password = $this->hash_password_db_super($user->id, $password);
	...
	public function superpatient($identity, $password, $remember = FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select('id , username, email, id, password, active, last_login')
		->where('id', $identity)
		->limit(1)
		->order_by('id', 'desc')
		->get('users');
		...
		if ($query->num_rows() === 1) {
			...
			$password = $this->hash_password_db_patient($user->id, $password);
	...
	public function appointmentcenterlogin($identity, $password, $remember = FALSE)
	{
		$this->trigger_events('pre_login');
		...
		if ($query->num_rows() === 1) {
			...
			$password = $this->hash_password_db_appointmentcenter($user->id, $password);
	...
	public function hash_password_db_super($id, $password, $use_sha1_override = FALSE)
	{
		if (empty($id) || empty($password)) {
			return FALSE;
		}
	...
	public function hash_password_db_patient($id, $password, $use_sha1_override = FALSE)
	{
		if (empty($id) || empty($password)) {
			return FALSE;
		}
	...
	public function hash_password_db_appointmentcenter($id, $password, $use_sha1_override = FALSE)
	{
		if (empty($id) || empty($password)) {
			return FALSE;
		}
	...
	public function is_time_locked_out($identity)
	{
		...
		$recent_attempt = $this->get_last_attempt_time($identity) > time() - $this->config->item('lockout_time', 'ion_auth') ? 1 : 0; // OTP RELATED CHANGE
		// echo "<script language=\"javascript\">alert(\"".$this->is_max_login_attempts_exceeded($identity)."\");</script>";
		// echo "<script language=\"javascript\">alert(\"".$recent_attempt."\");</script>";
		...
		return $this->is_max_login_attempts_exceeded($identity) && $recent_attempt;
	}
	...
	// public function increase_login_attempts_appointmentcenter($identity) {
	// if ($this->config->item('track_login_attempts', 'ion_auth')) {
	// $ip_address = $this->_prepare_ip($this->input->ip_address());
	// return $this->db->insert($this->tables['login_attempts'], array('ip_address' => $ip_address, 'login' => $identity, 'time' => time()));
	// }
	...
	// public function clear_login_attempts_appointmentcenter($identity, $expire_period = 86400) {
	// if ($this->config->item('track_login_attempts', 'ion_auth')) {
	// $ip_address = $this->_prepare_ip($this->input->ip_address());
	...
	public function unvalidate_otp($id)
	{
		// $this->trigger_events('update_last_login');
		...
		$this->db->update($this->tables['users'], array('otp_validated' => 0), array('id' => $id));

		return $this->db->affected_rows() == 1;
	}

	// public function update_last_login_appointmentcenter($id)
	// {
	// $this->trigger_events('update_last_login');
	...
	// $this->db->update($this->tables['appointmentcenterusers'], array('last_login' => time()), array('id' => $id));
	...
	public function set_session($user)
	{
		...
		$session_data = array(
			...
			'id_organisation'      => isset($user->id_organisation) ? $user->id_organisation : ''
		);
	...
	// public function remember_user_appointmentcenter($id)
	// {
	// $this->trigger_events('pre_remember_user');
	...
	// $this->db->update($this->tables['appointmentcenterusers'], array('remember_code' => $salt), array('id' => $id));
	...
	public function activateresetpwd($id, $activation_code)
	{
	...
	public function ion_authSuperadm($data)
	{
	...
	public function updatepasswordSadm($identity)
	{
		if (empty($identity)) {
			$this->trigger_events(array('post_forgotten_password', 'post_forgotten_password_unsuccessful'));
			//return FALSE;
		...
		$this->db->update('superusers', $update, array($this->identity_column => $identity));
	...
	public function clear_forgotten_password_codeSadm($code)
	{
		...
		//if ($this->db->count_all_results($this->tables['users']) > 0)
		{
			...
			$this->db->update('superusers', $data, array('forgotten_password_code' => $code));
	...
	public function reset_passwordSadm($identity, $new)
	{
		$this->trigger_events('pre_change_password');
		...
		$this->trigger_events('extra_where');
		$this->db->update('superusers', $data, array($this->identity_column => $identity));

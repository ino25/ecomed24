...
class Payu extends MX_Controller {
    ...
    public function check() {
        ...
        $invoice_id = $this->input->post('payment_id');
        ...
        $user = $this->ion_auth->get_user_id();
         $deposit_type = 'Cash';
          $data = array();
            $data = array('patient' => $payment->patient,
                'date' => $date,
 ...
 $this->finance_model->updateDeposit($invoice_id, $data);

                $amount_received_id = $this->finance_model->getDepositById($invoice_id)->amount_received_id;
                if (!empty($amount_received_id)) {
                 ...
                 $redirect = "patient/medicalHistory?id=" . $payment->patient;
                 redirect($redirect);
/*

        //payumoney details

        $payumoney = $this->pgateway_model->getPaymentGatewaySettingsByName('Cash');

...
$this->load->model('services/service_model');
        ...
        if (!$this->ion_auth->in_group(array('admin','adminmedecin', 'Doctor', 'Patient','adminmedecin'))) {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Doctor', 'Patient','adminmedecin'))) {
            redirect('home/permission');
        }
    ...
    function live() {
        ...
        $live_details = $this->meeting_model->getMeetingById($live_id,$this->id_organisation);
        ...
        $settings = $this->meeting_model->getMeetingSettingsById($doctor_ion_id,$this->id_organisation);
	...
	function save_notes() {
		...
		$added = $this->db->query("INSERT INTO `medical_history` (`patient_id`, `id_organisation`, `title`, `description`, `patient_name`, `patient_address`, `patient_phone`, `img_url`, `date`, `registration_time`, `date_string`, `payment_id`, `prestation`, `code`, `user`, `patient_last_name`) VALUES ($patient_id,$id_organisation , '$title_note', '$notes', '$patient_name', '', '$patient_phone', '', '$date_note', '', '', '1', '', '', '$doctor_id', '')");
	...
	function request_access(){
		...
		// echo "<script language=\"javascript\">alert(\"".$random_otp."\");</script>";
		// echo "<script language=\"javascript\">alert(\"".$patient_id."\");</script>";
		// echo "<script language=\"javascript\">alert(\"".$patient_phone."\");</script>";
		// echo "<script language=\"javascript\">alert(\"".$patient_email."\");</script>";
		
		$this->db->query("insert into generated_otp (user_id, mobile_number, email, otp, date_created, for_patient_record) VALUES(".$current_user_id.", \"".$patient_phone."\",\"".$patient_email."\",\"".$random_otp."\",\"".time()."\",".$patient_id.")");
	    ...
	    // $response['patient_phone'] = $patient_phone;
	...
	function resend_otp_patient_record(){
		// $otp = "123abc";
		$requestData = $_REQUEST;
        $patient_id = $requestData["patient_id"];
        $patient_phone = $requestData["patient_phone"];
	...
	function check_code(){
		...
		$row = $this->db->query("select id, (unix_timestamp() - date_created) as time_since_generation from generated_otp where user_id = ".$current_user_id." and otp=\"".$auth_code."\" and for_patient_record=".$patient_id." and is_valid")->row();
		$found = $row->id;
		...
		if($found) { 
			if($time_since_generation <= $time_before_expiration) { // OTP pas encore expiré
				...
				$this->db->query("update generated_otp set is_valid = 0 where user_id = ".$current_user_id." and otp = \"".$auth_code."\" and for_patient_record = ".$patient_id);
				
				
				$response['error'] = false;
				// $response['message'] = "Patient Record";
				$response['patient_record'] = $this->patient_model->getPatientById($patient_id,$this->id_organisation);
				if (!empty($response['patient_record']->birthdate)) {

					$today = date("Y-m-d");
					// $dateOfBirth = strtotime("$today -2 months -20 days");
					$dateOfBirth = strtotime($response['patient_record']->birthdate);
					$birthday = date("Y-m-d", $dateOfBirth);
					$diff = date_diff(date_create($today), date_create($birthday));
					// echo $diff->y;
					if ($diff->y > 0) {
						$response['age'] = $diff->y . ' An(s)';
					} else if ($diff->m > 0) {
						$response['age'] = $diff->m . ' Mois';
					} else if ($diff->d > 0) {
						$response['age'] = $diff->d . ' Jours';
					}
				}
				$response['medical_histories'] = $this->patient_model->getMedicalHistoryByPatientId($patient_id, $this->id_organisation);
				$response['labs'] = $this->lab_model->getLabByPatientId($patient_id);
				$response['patient_materials'] = $this->patient_model->getPatientMaterialByPatientId($patient_id, $this->id_organisation);
				// echo "<script language=\"javascript\">alert(\"Here I am still 3\");</script>";
				...
				$response["message"] = "<i class='fa fa-exclamation-triangle' style='font-size:30px;color:red;'></i><p style='width:80%;margin:auto;color:#000;text-align:center;font-size:95%;'>Le code de vérification a expiré. Merci de cliquer sur le bouton Renvoyer le code.</p>";
		...
		// $patient_record = $_POST['patient_record'];
		// if($otp == $auth_code){
			// $response['error'] = false;
			// $response['message'] = "Patient Record";
		// }else{
			// $response['error'] = true;
			// $response['message'] = "Authorization Faild";
		// }
		echo json_encode($response);
	}
    ...
    function jitsi() {
        $appointment_id = $this->input->get('id');
        $appointment_details = $this->appointment_model->getAppointmentById($appointment_id,$this->id_organisation);
        ...
        if ($this->ion_auth->in_group(array('Doctor','adminmedecin'))) {
           ...
           /* if ($doctor_id != $doctor) {
            ...
            $patient_details = $this->patient_model->getPatientByIonUserId($patient_ion_id,$this->id_organisation);
    ...
    function instantLive() {
        $appointment_id = $this->input->get('id');
         $data['settings'] = $this->settings_model->getSettings(); 
        if ($data['settings']->live_appointment_type == 'jitsi') {
        ...
        if ($this->ion_auth->in_group(array('Doctor','adminmedecin'))) {
           ...
           $doctor_ion_id = ''; if(isset($doctor_details)) { $doctor_ion_id = $doctor_details->ion_user_id; }
        }

        $doctorname = ''; if(isset($doctor_details)) { $doctorname = $doctor_details->name; }
...
// API Requests Ends
    ...
    public function addNew() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $patient_ion_id = $this->patient_model->getPatientById($patient,$this->id_organisation)->ion_user_id;
            $data = array();
            $data = array(
                'patient' => $patient,
    ...
    function editMeeting() {
        ...
        $data['patients'] = $this->patient_model->getPatientById($data['meeting']->patient,$this->id_organisation);
    ...
    function editMeetingByJason() {
        $id = $this->input->get('id');
        $data['meeting'] = $this->meeting_model->getMeetingById($id);
        $data['patient'] = $this->patient_model->getPatientById($data['meeting']->patient,$this->id_organisation);
    ...
    function getMeetingList() {
        ...
        foreach ($data['meetings'] as $meeting) {
            ...
            if ($status->status == 'started') {
                ...
                $patientdetails = $this->patient_model->getPatientById($meeting->patient,$this->id_organisation);
                ...
                if ($this->ion_auth->in_group(array('admin','adminmedecin', 'Doctor', 'Patient'))) {
                    $new_option = '';
                }

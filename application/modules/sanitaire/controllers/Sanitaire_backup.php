...
//        }
    ...
    function sarsCOV2() {
        ...
        $data['deposits'] = $this->finance_model->getDepositsByDate($date_from, $date_to);
        $data['expenses'] = $this->finance_model->getExpenseByDate($date_from, $date_to, $this->id_organisation);
        $data['sanitaires'] = $this->sanitaire_model->getSanitaireAllSARSCOVID2($this->id_organisation);
        ...
        if (!empty($typedepense)) {
            ...
            $date_fin = strtotime(str_replace('/', '-', $date_fin));
            $data['datePeriode'] = 'Du ' . $dateDebut . ' au ' . $dateFin;
            $data['sanitaires'] = $this->sanitaire_model->getTableSanitaireFilterTypeByLimitBySearchSARSCOVID2(10, 1, 'test', $this->id_organisation, $date_debut, $date_fin, $typedepense);
            $data['dateDebut'] = $dateDebut;
            $data['dateFin'] = $dateFin;
        } 
        else if ($dateDebut) {
            ...
            $date_fin = $dateFin . ' 23:59';
            $date_fin = strtotime(str_replace('/', '-', $date_fin));
            $data['sanitaires'] = $this->sanitaire_model->getSanitaireFilterDateSARSCOVID2($this->id_organisation, $date_debut, $date_fin);
            $data['datePeriode'] = 'Du ' . $dateDebut . ' au ' . $dateFin;
            $data['dateDebut'] = $dateDebut;
            $data['dateFin'] = $dateFin;
        } else {
    ...
    function getSanitaireCOVID() {
        ...
        //		}
                ...
                if ($limit == -1) {
                    if (!empty($search)) {
                        $data['sanitaires'] = $this->sanitaire_model->getTableSanitaireFilterTypeBySearchSARSCOVID2($search, $this->id_organisation, $date_debut, $date_fin, $typedepense);
                    } else {
                        $data['sanitaires'] = $this->sanitaire_model->getTableSanitaireFilterTypeSARSCOVID2($this->id_organisation, $date_debut, $date_fin, $typedepense);
                    }
                } else {
                    if (!empty($search)) {
                        $data['sanitaires'] = $this->sanitaire_model->getTableSanitaireFilterTypeByLimitBySearchSARSCOVID2($limit, $start, $search, $this->id_organisation, $date_debut, $date_fin, $typedepense);
                    } else {
                        $data['sanitaires'] = $this->sanitaire_model->getTableSanitaireFilterTypeByLimitSARSCOVID2($limit, $start, $this->id_organisation, $date_debut, $date_fin, $typedepense);
                    }
                }
        //        if ($Payment_encours) {
        //            $data['payments'] = $this->finance_model->getPaymentByFilter($this->id_organisation, $Payment_encours);
    ...
    //                 $output = array(
    //                     "draw" => intval($requestData['draw']),
    //                     "recordsTotal" => count($this->sanitaire_model->getTableSanitaireFilterTypeSARSCOVID2($this->id_organisation, $date_debut, $date_fin, $typedepense)),
    //                     "recordsFiltered" => count($this->sanitaire_model->getTableSanitaireFilterTypeBySearchSARSCOVID2($search, $this->id_organisation, $date_debut, $date_fin, $typedepense)),
    //                     "data" => $info
    //                 );
    //             } else if (!empty($data['sanitaires'])) {
    //                 $output = array(
    //                     "draw" => intval($requestData['draw']),
    //                     "recordsTotal" => count($this->sanitaire_model->getTableSanitaireFilterTypeSARSCOVID2($this->id_organisation, $date_debut, $date_fin, $typedepense)),
    //                     "recordsFiltered" => count($this->sanitaire_model->getTableSanitaireFilterTypeSARSCOVID2($this->id_organisation, $date_debut, $date_fin, $typedepense)),
    //                     "data" => $info
    //                 );
    //             } else {
    //                 $output = array(

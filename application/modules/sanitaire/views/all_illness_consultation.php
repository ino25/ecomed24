                ...
                <div class="adv-table editable-table">
                    ...
                    <div class="col-lg-12 col-md-12">
                        ...
                        <table border="0" cellspacing="5" cellpadding="5" style="float:left">
                            <tbody>
                                <tr>
                                    <td><strong style="color:#0d4d99"> Sexe: </strong></td>
                    ...
                    <div class="row">
                        <div class="col-md-12 payment pad_bot">
                            <label for="exampleInputEmail1"><strong style="color:#0d4d99"> TYPE / ANALYSE</strong> </label>
                            <select class="form-control m-bot15 js-example-basic-multiple conclusion" id="conclusion" name="conclusion" value='' onchange="conclusionUpdate(event)">
                                <option value="">Selectionner un type ou une analyse </option>
                                <?php foreach ($illness_conclusion as $template) { ?>
                    ...
                    <table class="table table-hover progress-table" id="editable-sample">
                        <thead>
                            <tr>
                                ...
                                <th> TYPE / ANALYSE </th>
                                <th>AFFECTION / ACTE </th>
                                <th>CONCLUSION / RESULTAT </th>
                                <th>ANCIENNETE </th>
                                <th>ADRESSE</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($illness as $sanitaire) { ?>
                                <tr class="">
                                    <td style="vertical-align:middle;"><?php echo date('d/m/Y', $sanitaire->createdDate); ?></td>
                                    ...
                                    <td style="vertical-align:middle;"><?php echo $sanitaire->name; ?></td>
                                    <td style="vertical-align:middle;"><?php echo $sanitaire->groupe; ?></td>
                                    <td style="vertical-align:middle;"><?php echo $sanitaire->sous_groupe; ?></td>
                                    <td style="vertical-align:middle;"><?php if($sanitaire->number_passage > 1  ) { echo 'Ancien'; } else  echo 'Nouveau'; ?></td>
                                    <td style="vertical-align:middle;"><?php echo $sanitaire->address; ?></td>
...
<script>
    ...
    function conclusionUpdate(eventObj) {

        var liste = document.getElementById('selectperiode');
        var myElementValue = liste.value;
        var listeType = document.getElementById('conclusion');
        var typeValue = listeType.value;
        updatePeriode(myElementValue);
        var recette = 'Masculin';
        var depense = 'Feminin';
        if (myElementValue != 'parPeriode') {
            if (typeValue.indexOf(recette) != -1) {
                var typeElement = 'Masculin';
                var url = `debut=${debut}&fin=${fin}&type=${typeElement}&conclusion=${typeValue}&periode=${myElementValue}`;
                window.location.search = url;
            } else if (typeValue.indexOf(depense) != -1) {
                var typeElement = 'Feminin';
                var url = `debut=${debut}&fin=${fin}&type=${typeElement}&conclusion=${typeValue}&periode=${myElementValue}`;
                ...
                var url = `debut=${debut}&fin=${fin}&conclusion=${typeValue}&periode=${myElementValue}`
                window.location.search = url;
            ...
            if (typeValue.indexOf(recette) != -1) {
                var typeElement = 'Masculin';
                var url = window.location.search + `&type=${typeElement}&conclusion=${typeValue}`;
                window.location.search = url;
            } else if (typeValue.indexOf(depense) != -1) {
                var typeElement = 'Feminin';
                var url = window.location.search + `&type=${typeElement}&conclusion=${typeValue}`;
    ...
    $('.social-option select').on('change', function(eventObj) {
        ...
        var listeType = document.getElementById('modeType');
        var typeValue = listeType.value;
        var listeTypeConclusion = document.getElementById('conclusion');
        var typeValueConclusion = listeType.value;
        if (eventObj.target.value != 'parPeriode') {

            var url = `debut=${debut}&fin=${fin}&periode=${eventObj.target.value}`
            window.location.search = url;
            document.querySelector('#modeType').value = urlParams.get('type') == 'Feminin' ? 'Feminin' : urlParams.get('type') == 'Masculin' ? 'Masculin' : '';
            document.querySelector('#conclusion').value = urlParams.get('conclusion')
...
<script>
    ...
    $(document).ready(function() {
        ...
        document.querySelector('#modeType').value = urlParams.get('type') == 'Masculin' ? 'Masculin' : urlParams.get('type') == 'Feminin' ? 'Feminin' : '';
        document.querySelector('#conclusion').value = urlParams.get('conclusion') == '' ? 'Tout' : urlParams.get('conclusion')
        var table = $('#editable-sample').DataTable({
            ...
            buttons: {
                buttons: [{
                    extend: 'excelHtml5',
                    title: 'rapport_activité_' + organisation + '_' + date_debut + '-' + date_fin,

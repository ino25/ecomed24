...
class Finance extends MX_Controller
{
                ...
                if ($valueTab[5] == 'lab') {
                    ...
                    if (empty($report_exist)) {
                        ...
                        if (empty($report_exist->transfer)) {
                            ...
                            $status = '<span class="status-p bg-primary">ENVOYÉ</span>';
                            $count_valid++;
                            if ($this->ion_auth->in_group(array('adminmedecin', 'Doctor', 'Receptionist', 'Assistant'))) {
                                $options2 = '';
                                $options1 = '  <a id="done' . $valueTab[0] . '" class="green btn payment" href="lab/reportViewResultat?id=' . $report_exist->id . '" title="Résultat"><i class="fa fa-eye"></i></a>';
    ...
    function editStatutPaiementByJasonValid()
    {
        ...
        $this->finance_model->updatePCR($pcr, array('date_rendu' => $date_string, 'signature' => $data['signature']));
    ...
    function generateRapportPDF()
    {
        ...
        $origins = $this->partenaire_model->payButtonJson($payment, $this->id_organisation);
        $patientOrigins = $origins->patient;
        $tabuser = $this->home_model->getUserById($origins->user);
        $organisationID = $this->finance_model->getOrganisationJson($origins->id_organisation);
        $patient = $this->patient_model->getPatientById($patientOrigins, $this->id_organisation);
        ...
        $data['organisationID'] = $origins->id_organisation;
        $data['doctorList'] = $doctorsList;
        $data['organisationAddress'] = $origins->organisationAddress;
        $data['resultQR'] = base_url() . "uploads/qrcode/" . $origins->resultQR;
        $data['organisationLogo'] = $this->entete;
        $anneeEncours = date("Y");
        ...
        $img_file_resultQR = $data['resultQR'];
        $imgData_resultQR = base64_encode(file_get_contents($img_file_resultQR));
        ...
        $data['orderingOrganisationName'] = $origins->orderingOrganisationName;
        $data['orderNumber'] = $origins->orderNumber;
        $data['clinicalNotes'] = $origins->renseignementClinique;
        //$data['doctorSignature'] = $origins->signature;
        $data['doctorSignature'] = $this->signature;
        $img_file_signature = $data['doctorSignature'];
        $imgData_signature = base64_encode(file_get_contents($img_file_signature));
    ...
    function generateRapportPDFPCR()
    {
        ...
        $payment = $this->input->get('id');
        $origins = $this->partenaire_model->payButtonJsonPCR($payment, $this->id_organisation);
        $patientOrigins = $origins->patient;
        $tabuser = $this->home_model->getUserById($origins->user);
        $organisationID = $this->finance_model->getOrganisationJson($origins->id_organisation);
        $patient = $this->patient_model->getPatientById($patientOrigins, $this->id_organisation);
    ...
    function genererDocument()
    {
        ...
        $idpayment = utf8_encode($_POST['idpayment']); // Don't forget the encoding

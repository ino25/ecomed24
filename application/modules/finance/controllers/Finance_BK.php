					...
					if ($valueTab[4] == 0) {
						...
						if ($payment->etatlight && ($payment->status == "valid")) {
							...
							if ($this->ion_auth->in_group(array('Laboratorist', 'adminmedecin', 'Doctor', 'admin', 'Biologiste'))) {
								$options1 = '  <a id="valid' . $id_prestation . '" class="green btn payment" href="finance/getResultatIBydActe?id=' . $payment->patient . '&payment=' . $payment->id . '&prestation=' . $id_prestation . '&service=' . $code_service . '&modele=' . $modele . '&nameservice=' . $service_name . '&emaillight=' . $email_organisation_light_origine . '&idlight=' . $id_organisation_light .  '&nomlight=' . $nom_organisation_light_origine . '"  ><i class="fa fa-eye"></i> ' . lang('view_report') . '</a>';
						...
						if (empty($report_exist->transfer)) {
							if ($report_exist->status === "Valider") {
								...
								if ($this->ion_auth->in_group(array('adminmedecin', 'Doctor', 'Receptionist', 'Assistant'))) {
									$options2 = '';
									$options1 = '  <a id="done' . $valueTab[0] . '" class="green btn payment" href="lab/reportViewResultat?id=' . $report_exist->id . '"><i class="fa fa-eye"></i> ' . lang('view_report') . '</a>';
									//  $options1 = '';
...
//						);
	...
	function getPaymentLabo()
	{
		...
		//         $valueTab = explode('*', $value);
		//         if (count($valueTab) == 5) {
		//             var_dump($valueTab[5].'premier if valeur 5');
		//             exit();
		//             $valueTab[5] = ' ';
		//         }
...
//        }
	...
	function editStatutPaiementByJasonValid()
	{
		...
		$data_up = array('signature' => 'yes', 'doctor_name' => $doctor, 'date_rendu' => $date_string, 'qr_code' => $data_get['file']);
		$date_string = date('d-m-Y H:i', $date);
...
<div style="text-align:right;font-weight: bold; font-size: 8pt; font-style: italic;">' .
	...
	function getResultatIBydActe()
	{
		...
		if (empty($data['origins']->signature)) {
			$data['origins']->signature = 'uploads/entetePartenaires/signatureDefault.png';
		}

...
class Lab extends MX_Controller
{
    ...
    public function addLabNewPayment()
    {
        ...
        $dataPcr = array();
        $id_pcr = $this->finance_model->getTestPcr($id_presta);
        $name_specialite = $this->finance_model->prestationDetailsByID($id_presta);
        $name_specialite = $name_specialite->name_specialite;
        $dataPcr = array(
            'payment' => $payment,
            'resultat' => isset($resultats[0]) ? $resultats[0] : null,
            'conclusion' => isset($resultats[1]) ? $resultats[1] : null,
            'type_de_prelevement' => isset($resultats[2]) ? $resultats[2] : null,
            'heure_de_prelevement' => $date_prelevement,
            'prestation' => $id_pcr->prestation,
            'specialite' => $name_specialite,

        );
        $dataPcrMolecule = array(
            'payment' => $payment,
            'resultat' => isset($resultats[1]) ? $resultats[1] : null,
            'conclusion' => isset($resultats[3]) ? $resultats[3] : null,
            'type_de_prelevement' => isset($resultats[2]) ? $resultats[2] : null,
            'note' => isset($resultats[0]) ? $resultats[0] : null,
            'heure_de_prelevement' => $date_prelevement,
            'prestation' => $id_pcr->prestation,
            'specialite' => $name_specialite,

        );
        $dataPcrMoleculeHumaine = array(
            'payment' => $payment,
            'resultat' => isset($resultats[0]) ? $resultats[0] : null,
            'conclusion' => isset($resultats[1]) ? $resultats[1] : null,
            'type_de_prelevement' => isset($resultats[2]) ? $resultats[2] : null,
        ...
        if (empty($PCR)) {
            ...
            if ($id_pcr->code_prestation == 'BIOMED275') {
                $this->finance_model->insertPCR($dataPcr);
            } else if ($id_pcr->code_prestation == 'BIOMED276') {
                $this->finance_model->insertPCR($dataPcrMolecule);
            } else if ($id_pcr->code_prestation == 'BIOMED277') {
                $this->finance_model->insertPCR($dataPcrMoleculeHumaine);
            }
        } else {
            if ($id_pcr->code_prestation == 'BIOMED275') {
                $this->finance_model->updatePCR($PCR, $dataPcr);
            } else if ($id_pcr->code_prestation == 'BIOMED276') {
                $this->finance_model->updatePCR($PCR, $dataPcrMolecule);
            }else if ($id_pcr->code_prestation == 'BIOMED277') {
                $this->finance_model->updatePCR($PCR, $dataPcrMolecule);
            }
        }
        ...
        $this->finance_model->updateTransactionStatus($id_presta, $payment, array('status' => "EFFECTUÉ"));

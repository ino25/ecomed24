    ...
    <section class="wrapper site-min-height">
        ...
        <section class="panel">
            ...
            <header class="panel-heading" style="margin-top:41px;">
                <h2><?php echo lang('auto_whatsapp_template'); ?></h2>
            </header>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form role="form" id="whatsapptemp" name="myform" action="whatsapp/addNewAutoWhatsappTemplate" method="post" enctype="multipart/form-data">                                                                                    
...
<script type="text/javascript">
    $(document).ready(function () {
        $(".table").on("click", ".editbutton1", function () {
            ...
            $.ajax({
                url: 'whatsapp/editAutoWhatsappTemplate?id=' + iid,
                method: 'GET',
                data: '',
                dataType: 'json',
            }).success(function (response) {
                // Populate the form fields with the data returned from server
                $('#whatsapptemp').find('[name="id"]').val(response.autotemplatename.id).end();
                $('#whatsapptemp').find('[name="category"]').val(response.autotemplatename.name).end();
             //   CKEDITOR.instances['editor1'].setData(response.autotemplatename.message);
             $('#whatsapptemp').find('[name="message"]').val(response.autotemplatename.message).end();
                var option = '';
                var count = 0;
...
<script>
    ...
    $(document).ready(function () {
        var table = $('#editable-sample1').DataTable({
            ...
            "searchable": true,
            "ajax": {
                url: "whatsapp/getAutoWhatsappTemplateList",
                type: 'POST',
                'data': {'type': 'whatsapp'}
            },
            scroller: {

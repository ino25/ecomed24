        ...
        if ($this->ion_auth_model->forgotten_password($identity)) {   //changed
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
					...
					$data1Email = array(
						'identity' => $user->{$this->config->item('identity', 'ion_auth')},
						'reset_url' => base_url().'auth/reset_password/'.$user->forgotten_password_code
					);
...
// <body>
	...
	// <p>'. sprintf(lang('email_forgot_password_subheading'), anchor('auth/reset_password/'. $user->forgotten_password_code, lang('email_forgot_password_link'))) .'</p>
// </body>
// </html>';
                    // $dataInsert_old = array(
                        // 'reciepient' => $identity,
                        // 'subject' => $subject,
                        // 'message' => $message_old,
                        // 'date' => time(),
                    ...
                    // $this->email_model->insertEmail($dataInsert_old);
                    $this->set_message('forgot_password_successful');
                    return TRUE;
    ...
    /**
     * reset_activation feature
     *
     * @return mixed  boolian / array
     * @author Mathew
     * */
    public function reset_activation($identity, $username, $company) {    //changed $email to $identity
        if ($this->ion_auth_model->forgotten_password($identity)) {   //changed
            // Get user information
            $identifier = $this->ion_auth_model->identity_column; // use model identity column, so it can be overridden in a controller
            // $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  //changed to get_user_by_identity from email
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
					...
					$autoemail = $this->email_model->getAutoEmailByType('reset_activation');
					$message1 = $autoemail->message;
					$subject = $this->parser->parse_string($autoemail->name, $data1Email, TRUE);
					// $subject = $this->lang->line('email_forgotten_password_subject')." ecoMed24";
					$messageprint2 = $this->parser->parse_string($message1, $data1Email, TRUE);
...
// </html>';
    ...
    public function register($username, $password, $email, $additional_data = array(), $group_ids = array()) { //need to test email activation
        ...
        if (!$email_activation) {
            ...
            ////rf compte desactive a la creation
            ...
            $user = $this->where($email_activation, $email_activation)->users()->row();

         
            $data1Email = array(
               // 'identity' => $user->{$identity},
               //'activation_url' => base_url().'auth/activate/'. $user->id .'/'. $activation_code,
                'identity' => $user->{$this->config->item('identity', 'ion_auth')},
				'activation_url' => base_url().'auth/reset_password/'.$user->forgotten_password_code,
                'id' => $user->id,
                'email' => $email,
                'activation_code' => $this->ion_auth_model->activation_code
            );
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                // $message = $this->load->view($this->config->item('email_templates', 'ion_auth') . $this->config->item('email_activate', 'ion_auth'), $data, true);
                $subject = $this->lang->line('email_activation_subject')." ecoMed24";

				
				$autoemail = $this->email_model->getAutoEmailByType('account_activation_by_email');
				$message1 = $autoemail->message;
				...
				// }
                 // $message_old = '<html>
// <body>
	// <h1>'. sprintf(lang('email_forgot_password_heading'), $email) .'</h1>
	// <p>'. sprintf(lang('email_forgot_password_subheading'), anchor('auth/activate/'. $id.'/'. $activation_code, lang('email_forgot_password_link'))) .'</p>
// </body>
// </html>';
                    // $dataInsert_old = array(
                        // 'reciepient' => $email,
    ...
    public function in_group($check_group, $id = false, $check_all = false) {
     ...
     public function forgotten_password_checkSadm($code) {
        ...
        if (!is_object($profile)) {
            ...
            if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0) {
                ...
                if (time() - $profile->forgotten_password_time > $expiration) {
                    //it has expired
                    $this->clear_forgotten_password_codeSadm($code);
                    $this->set_error('password_change_unsuccessful');
                    return FALSE;
                }
            }

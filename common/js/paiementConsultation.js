    ...
    $('#pos_select').change(function () {
        ...
        if ($("#spartenaire").val() == null || $("#spartenaire").val() == '') {
            ...
            if (id != 'add_new') {
                ...
                $.ajax({
                    url: 'finance/editMutuelleFinanceConsultationLabo?patient=' + id,
                    method: 'GET',
                    data: '',
                    dataType: 'json',
                    // delai: 250
                    ...
                    if ($('#id-div' + idd).length) {
                        ...
                        if (!is_categoryinput) {
                            ...
                            if (lightpartenaire) {
                                ...
                                var liste_article = '<div class="col-md-12 left0" id="divv' + idd + '" >';
                                liste_article += '<div class="col-md-10 remove1_" id="id-div' + idd + '" id="id-remise' + idd + '">  ' + $(this).data("spec_name") + " > " + $(this).data("cat_name") + '</div>';
...
$(document).ready(function () {
    ...
    $("#add_service").select2({
        ...
        ajax: {
            ...
            url: 'services/getServicesCoverageByConsultationJson',
            type: "post",
            dataType: 'json',
            delay: 250,
            data: function (params) {
...
function chargeListePrestation(search = '') {
    ...
    if (!spartenaire) {
        ...
        if (patient && patient != 'add_new') {
            ...
            $.ajax({
                url: 'finance/editTproFinanceByJasonLabo?patient=' + patient + '&id_service_specialite_organisation=' + id_service_specialite_organisation + "&id_assurance=" + assurance_id + "&search=" + search, // CHANGE DONE
                method: 'GET',
                data: '',
				async: false,
                dataType: 'json',
        ...
        $.ajax({
            ...
            url: 'finance/editTproFinanceByJasonLabo?organisation=' + spartenaire + '&id_service_specialite_organisation=' + id_service_specialite_organisation + "&search=" + search + "&id_organisation=" + id_organisation,
            method: 'GET',
            data: '',
            dataType: 'json',
            // delai: 250
            ...
            if (response.services) {
                $.each(response.services, function (key, value) {
                    ...
                    $('select.multi-select').append($('<option class="ooppttiioonn" data-id="' + tarif_public + '" data-idd="' + value.id + '" data-spec_name="' + value.name_specialite + '" data-cat_name="' + value.prestation + '"  data-remise="0"  >' + value.prestation + '</option>').text(value.name_specialite + " > " + value.prestation).val(value.name_specialite + " > " + value.prestation));
                });
            }
            $('.multi-select').multiSelect('refresh');
            $('.ms-selectable input.search-input').attr('placeholder', 'Selectionnez une prestation');

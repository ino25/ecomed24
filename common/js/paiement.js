$(document).ready(function () {
    $('#dis_id').keyup(function () {
        ...
        ggggg = ggggg + montant_service;

        $('#editPaymentForm').find('[name="grsss"]').val(ggggg)
        $('#editPaymentForm').find('[name="montant_service_result"]').val(ggggg)
...
$(document).ready(function () {
    ...
    $('.add_lab_test').change(function () {
        chargeListePrestation1();
    ...
    $('#passport_info').submit(function () {

        var specify = $('#purpose').val();
        var type_pf = $('.type_pf').val();

        var passport = $('#passport').val();

        $('#passport_update').val(passport).end();
        $('#type_of_new').val(specify).end();
        $('.type_pf_purpose').val(type_pf);
        $('#pcr_exist').val('yes_pcr');
        $('#myModal3').modal('hide');
    });

    $(document.body).on('change', '#add_service', function () {
            ...
            if (id != 'add_new') {
                ...
                $.ajax({
                    ...
                    $.each(response.services, function (key, value) {
                        ...
                        $('select.multi-select').append($('<option class="ooppttiioonn" data-id="' + price_final + '" data-pro="' + price_pro + '" data-idd="service-' + value.id + '" data-spec_name="' + value.name_specialite + '" data-cat_name="' + value.prestation + '"  data-remise="' + remise + '"  >' + value.prestation + '</option>').text(value.prestation).val("service> " + value.id));

                    });
                    if (response.labs) {

                        $.each(response.labs, function (key, value) {

                            var price_final = parseInt(value.add_price);
                            var price_pro = parseInt(value.add_price);
                            var remise = 0;



                            price_final = parseInt(value.add_price);
                            //  price_final = parseInt(value.tarif_assurance) -  parseInt(value.tarif_assurance) * parseInt(charge)/100;
                            remise = parseInt(parseInt(value.add_price) * parseInt(charge) / 100);



                            $('select.multi-select').append($('<option class="ooppttiioonn" data-id="' + price_final + '" data-pro="' + price_pro + '" data-idd="lab-' + value.id + '" data-spec_name="' + value.speciality + '" data-cat_name="' + value.name + '"  data-remise="' + remise + '"  >' + value.name + '</option>').text(value.name).val("lab>" + value.id));
    ...
    $('.multi-select').change(function () {
        ...
        if ($('#pos_select').val()) {
            ...
            var latest_value = ' ';
            latest_value = $("#my_multi_select3 option:selected").last().val();
           
            var ids = [];
            if (latest_value != null) {
                ids = latest_value.split('>');
                if (ids[1]) {
                    
                    $.ajax({
                        url: 'finance/getTestPCR?id=' + ids[1],
                        method: 'GET',
                        data: '',
                        dataType: 'json'

                    }).success(function (response1) {
                    console.log("**** response PCR ****");
                    console.log(response1);
                        if (response1.response === 'BIOMED275') {
                            if ($('#passport_update').val() == ' ') {
                                $('#myModal3').modal({ backdrop: 'static', keyboard: false }, 'show');
                            }
                        }
                    });
                }
                    ...
                    if ($('#id-div' + idd).length) {
                        ...
                        if (!is_categoryinput) {
                            ...
                            if (lightpartenaire) {
                                ...
                                liste_article += '<i  class="col-md-1 fa fa-trash remove pull-left  delete" id="delete-' + idd + '"  data-val="' + val + '"    data-remise="' + remise + '"  data-price="' + price + '" onclick="deletePresta(\'' + idd_split[0] + '\',\'' + idd_split[1] + '\')"/>';
                                ...
                                liste_article += '<input type="hidden" class="remove" id="categoryinput1-' + idd + '" name="type[]" value="' + idd_split[0] + '">';
                                liste_article += '</div>';
                                ...
                                var liste_article = '<div class="col-md-12 left0" id="divv' + idd + '" >';
                                liste_article += '<div class="col-md-4 remove1_" id="id-div' + idd + '" id="id-remise' + idd + '">  '+ $(this).data("cat_name") + '</div>';
...
$(document).ready(function () {
    checklight = $('#checklight').val();
    partenairelightcheck = $('#partenairelightcheck').val();
    urlLight = "finance/addPaymentView?partenairelightcheck="+partenairelightcheck+"&idpartenairelightcheck="+checklight;
    ...
    if(checklight){
       // alert("test");
        ...
        if (checklight) {
            $.ajax({
                url: 'patient/editOrganisationByJason?id=' + checklight,
                method: 'GET',
                data: '',
                async: false,
                dataType: 'json',
                ...
                var name = response.organisation.nom;
                
                var organisationLight = new Option(name, response.organisation.id, true, true);
                console.log("******Retour Name *****");
                console.log(organisationLight);
                console.log("*******************");
                $('#editPaymentForm').find('[name="lightpartenaire"]').append(organisationLight).trigger('change');
            });
...
$(document).ready(function () {
    ...
    $(document.body).on('change', '#lightpartenaire', function () {

        var v = $("select.lightpartenaire option:selected").val();
        if (v == 'add_new') {
            $('#myModalLight').trigger("reset");
            $('#myModalLight').modal('show');
        } else {

            $('#myModalLight').modal('hide');
        }
    });
...
$(document).ready(function () {
    ...
    $('.ms-selectable input.search-input').attr('placeholder', 'Selectionnez une prestation');
    $("#add_lab_test").select2({
        placeholder: 'Sélectionnez Test de laboratoire',
        allowClear: true,
        ajax: {
            // url: 'services/getSericeinfoByJason',
            url: 'lab/getLabListForPayment',
            type: "post",
            dataType: 'json',
            delay: 250,
            data: function (params) {
...
function LightUpdate(eventObj) {
    if (eventObj.target.value == 'add_new') {
        $('#myModalLight').trigger("reset");
        $('#myModalLight').modal('show');
    } else {
        $('#myModalLight').modal('hide');
...
function chargeListePrestation1(search = '') {
    //alert("case 3");
    var spartenaire = document.getElementById("choicepartenaire").checked;
    var id_service_specialite_organisation = $('.add_lab_test').val();
    ...
    if (!spartenaire) {
        ...
        if (patient && patient != 'add_new') {
            ...
            $.ajax({
                url: 'finance/editMutuelleFinanceByJason?patient=' + patient + '&id_service_specialite_organisation=' + id_service_specialite_organisation + "&id_assurance=" + assurance_id + "&search=" + search + '&from=lab', // CHANGE DONE
                method: 'GET',
                data: '',
                async: false,
                dataType: 'json',
                ...
                $.each(response.labs, function (key, value) {
                    ...
                    $('select.multi-select').append($('<option class="ooppttiioonn" data-id="' + parseInt(value.add_price) + '" data-pro="' + price_pro + '" data-idd="lab-' + value.id + '" data-spec_name="' + value.speciality + '" data-cat_name="' + value.name + '"  data-remise="' + remise + '"  >' + value.name + '</option>').text(value.speciality).val("lab> " + value.id));
      ...
      //  alert("je ne suis pas selectionner");
        $('.multi-select').multiSelect('refresh');
        $('.multi-select').multiSelect('deselect_all');
        $.ajax({
            ...
            url: 'finance/editTproFinanceByJason?organisation=' + spartenaire + '&id_service_specialite_organisation=' + id_service_specialite_organisation + "&search=" + search + "&id_organisation=" + id_organisation + '&from=lab',
            method: 'GET',
            data: '',
            dataType: 'json',
            // delai: 250
            ...
            if (response.services) {
                $.each(response.services, function (key, value) {
                    ...
                    $('select.multi-select').append($('<option class="ooppttiioonn" data-id="' + tarif_public + '" data-idd="service-' + value.id + '" data-spec_name="' + value.name_specialite + '" data-cat_name="' + value.prestation + '"  data-remise="0" data-facture="0" >' + value.prestation + '</option>').text(value.prestation).val("service> " + value.id));
                });
            }
            if (response.labs) {
                $.each(response.labs, function (key, value) {

                    var tarif_professionnel = parseInt(value.add_price);
                    var tarif_public = parseInt(value.add_price);

                    $('select.multi-select').append($('<option class="ooppttiioonn" data-id="' + tarif_public + '" data-idd="lab-' + value.id + '" data-spec_name="' + value.name_specialite + '" data-cat_name="' + value.prestation + '"  data-remise="0"  data-facture="0" >' + value.prestation + '</option>').text(value.prestation).val("lab> " + value.id));
                });
            }
            $('.multi-select').multiSelect('refresh');
            $('.ms-selectable input.search-input').attr('placeholder', 'Selectionnez une prestation');
...
//alert("je suis selectionner")
        ...
        if (patient && patient != 'add_new') {
            ...
            $.ajax({
                url: 'finance/editMutuelleFinanceByJason?patient=' + patient + '&id_service_specialite_organisation=' + id_service_specialite_organisation + "&id_assurance=" + assurance_id + "&search=" + search + '&from=service', // CHANGE DONE
                method: 'GET',
                data: '',
                async: false,
                dataType: 'json',
...
//alert("je ne suis pas selectionner");
        ...
        $.ajax({
            // url: 'finance/editTproFinanceByJason?organisation=' + spartenaire + '&service=' + idServicePrestationSpecialite, // CHANGEMENT A FAIRE EGALEMENT
            url: 'finance/editTproFinanceByJason?organisation=' + spartenaire + '&id_service_specialite_organisation=' + id_service_specialite_organisation + "&search=" + search + "&id_organisation=" + id_organisation + '&from=service',
            method: 'GET',
            data: '',
            dataType: 'json',
            // delai: 250
            ...
            if (response.services) {
                $.each(response.services, function (key, value) {
                    ...
                    $('select.multi-select').append($('<option class="ooppttiioonn" data-id="' + tarif_public + '" data-idd="service-' + value.id + '" data-spec_name="' + value.name_specialite + '" data-cat_name="' + value.prestation + '"  data-remise="0"  >' + value.prestation + '</option>').text(value.prestation).val("service> " + value.id));
                });
            }
            if (response.labs) {
                $.each(response.labs, function (key, value) {
                    ...
                    $('select.multi-select').append($('<option class="ooppttiioonn" data-id="' + tarif_public + '" data-idd="lab-' + value.id + '" data-spec_name="' + value.name_specialite + '" data-cat_name="' + value.prestation + '"  data-remise="0"  >' + value.prestation + '</option>').text(value.prestation).val("lab> " + value.id));
                });
            }
            $('.multi-select').multiSelect('refresh');
            $('.ms-selectable input.search-input').attr('placeholder', 'Selectionnez une prestation');
...
function deletePresta(from, idds) {
    ...
    if (from === 'lab') {
        ...
        $.ajax({
            ...
            // url: 'finance/editTproFinanceByJason?organisation=' + spartenaire + '&service=' + idServicePrestationSpecialite, // CHANGEMENT A FAIRE EGALEMENT
            url: 'lab/getMasterLabIdForPcr?id=' + idds,
            method: 'GET',
            data: '',
            dataType: 'json',
            // delai: 250
        }).success(function (response) {
            if (response.response == 'pcr') {
                $('#purpose').val(' ');
                $('.type_pf').val(' ').trigger('change');
                $('#passport').val(' ');
                $('#passport_update').val(" ");
                $('#type_of_new').val(" ")
                $('#type_pf_purpose').val(' ').trigger('change');
                $('#pcr_exist').val(' ');
            }
        });
    }
